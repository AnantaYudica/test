...
{
    {
        ...
        const test::Node<Obj1, 1>& cnode1 = node1;
        assert(node1->val1 == 0);
        assert(node1.Link() == nullptr);
        assert(cnode1.Link() == nullptr);

        test::Node<Obj1, 1> node2{Obj1{2}};
        const test::Node<Obj1, 1>& cnode2 = node2;
        node2.Link(&node1);

        assert(node2->val1 == 2);
        assert(node2.Link() == &node1);
        assert(cnode2.Link() == &node1);

        Obj1 obj1{5};
        test::Node<Obj1, 1> node3{std::move(obj1)};
        const test::Node<Obj1, 1>& cnode3 = node3;
        node3.Link(&node1);
        
        assert(node3->val1 == 5);
        assert(node3.Link() == &node1);
        assert(cnode3.Link() == &node1);

        assert(obj1.val1 == 0);

        test::Node<Obj1, 1> node4{1 , 3};
        const test::Node<Obj1, 1>& cnode4 = node4;
        
        assert(node4->val1 == 4);
        assert(node4.Link() == nullptr);
        assert(cnode4.Link() == nullptr);

        test::Node<Obj1, 1> node5{node2};
        const test::Node<Obj1, 1>& cnode5 = node5;

        assert(node5->val1 == 2);
        assert(node5.Link() == &node1);
        assert(cnode5.Link() == &node1);

        test::Node<Obj1, 1> node6{std::move(node3)};
        const test::Node<Obj1, 1>& cnode6 = node6;

        assert(node6->val1 == 5);
        assert(node6.Link() == &node1);
        assert(cnode6.Link() == &node1);

        assert(node3->val1 == 0);
        assert(node3.Link() == nullptr);

        test::Node<Obj1, 1> node7;
        const test::Node<Obj1, 1>& cnode7 = node7;
        node7 = node2;
        
        assert(node7->val1 == 2);
        assert(node7.Link() == &node1);
        assert(cnode7.Link() == &node1);
        
        test::Node<Obj1, 1> node8;
        const test::Node<Obj1, 1>& cnode8 = node8;
        node8 = std::move(node6);

        assert(node8->val1 == 5);
        assert(node8.Link() == &node1);
        assert(cnode8.Link() == &node1);

        assert(node6->val1 == 0);
        assert(node6.Link() == nullptr);

        test::Node<Obj1, 1> node9;
        const test::Node<Obj1, 1>& cnode9 = node9;
        node9 = Obj1{9};
        
        assert(node9->val1 == 9);
        assert(node9.Link() == nullptr);
        assert(cnode9.Link() == nullptr);

        Obj1 obj2{100};
        test::Node<Obj1, 1> node10;
        const test::Node<Obj1, 1>& cnode10 = node10;
        node10 = std::move(obj2);
        
        assert(node10->val1 == 100);
        assert(node10.Link() == nullptr);
        assert(cnode10.Link() == nullptr);

        assert(obj2.val1 == 0);
    }
    {
        test::Node<Obj1, 2> node1;
        const test::Node<Obj1, 2>& cnode1 = node1;
        {
            auto node1_next = node1.Link();
            auto node1_prev = static_cast<test::Node<Obj1, 2, 0>&>(node1).Link();
            auto cnode1_next = cnode1.Link();
            auto cnode1_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode1).Link();
            assert(node1->val1 == 0);
            assert(node1_next == nullptr);
            assert(node1_prev == nullptr);
            assert(cnode1_next == nullptr);
            assert(cnode1_prev == nullptr);
        }

        test::Node<Obj1, 2> node2{Obj1{2}};
        const test::Node<Obj1, 2>& cnode2 = node2;
        node2.Link(&node1);
        static_cast<test::Node<Obj1, 2, 0>&>(node1).Link(&node2);
        {
            ...
            auto cnode1_next = cnode1.Link();
            auto cnode1_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode1).Link();
            auto cnode2_next = node2.Link();
            auto cnode2_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode2).Link();
            ...
            assert(cnode2_next == &node1);
            assert(cnode2_prev == nullptr);
            
            assert(cnode1_next == nullptr);
            assert(cnode1_prev == &node2);
        }
        
        Obj1 obj1{5};
        test::Node<Obj1, 2> node3{std::move(obj1)};
        const test::Node<Obj1, 2>& cnode3 = node3;
        node3.Link(&node2);
        static_cast<test::Node<Obj1, 2, 0>&>(node2).Link(&node3);
        {
            ...
            auto cnode2_next = cnode2.Link();
            auto cnode2_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode2).Link();
            auto cnode3_next = cnode3.Link();
            auto cnode3_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode3).Link();
            ...
            assert(cnode2_next == &node1);
            assert(cnode2_prev == &node3);
            
            assert(cnode3_next == &node2);
            assert(cnode3_prev == nullptr);
        }

        assert(obj1.val1 == 0);

        test::Node<Obj1, 2> node4{1 , 3};
        const test::Node<Obj1, 2>& cnode4 = node4;
        {
            auto node4_next = node4.Link();
            auto node4_prev = static_cast<test::Node<Obj1, 2, 0>&>(node4).Link();
            auto cnode4_next = cnode4.Link();
            auto cnode4_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode4).Link();

            assert(node4->val1 == 4);
            assert(node4_next == nullptr);
            assert(node4_prev == nullptr);
            
            assert(cnode4_next == nullptr);
            assert(cnode4_prev == nullptr);
        }

        test::Node<Obj1, 2> node5{node2};
        const test::Node<Obj1, 2>& cnode5 = node5;
        {
            auto node5_next = node5.Link();
            auto node5_prev = static_cast<test::Node<Obj1, 2, 0>&>(node5).Link();
            auto cnode5_next = cnode5.Link();
            auto cnode5_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode5).Link();

            assert(node5->val1 == 2);
            assert(node5_next == &node1);
            assert(node5_prev == &node3);

            assert(cnode5_next == &node1);
            assert(cnode5_prev == &node3);
        }

        test::Node<Obj1, 2> node6{std::move(node5)};
        const test::Node<Obj1, 2>& cnode6 = node6;
        {
            auto node5_next = node5.Link();
            auto node5_prev = static_cast<test::Node<Obj1, 2, 0>&>(node5).Link();
            ...
            auto cnode5_next = cnode5.Link();
            auto cnode5_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode5).Link();
            auto cnode6_next = cnode6.Link();
            auto cnode6_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode6).Link();
            ...
            assert(cnode6_next == &node1);
            assert(cnode6_prev == &node3);
            assert(cnode5_next == nullptr);
            assert(cnode5_prev == nullptr);
        }

        test::Node<Obj1, 2> node7;
        const test::Node<Obj1, 2>& cnode7 = node7;
        node7 = node2;
        {
            auto node7_next = node7.Link();
            auto node7_prev = static_cast<test::Node<Obj1, 2, 0>&>(node7).Link();
            auto cnode7_next = cnode7.Link();
            auto cnode7_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode7).Link();

            assert(node7->val1 == 2);
            assert(node7_next == &node1);
            assert(node7_prev == &node3);

            assert(cnode7_next == &node1);
            assert(cnode7_prev == &node3);
        }
        
        test::Node<Obj1, 2> node8;
        const test::Node<Obj1, 2>& cnode8 = node8;
        node8 = std::move(node6);
        {
            auto node6_next = node6.Link();
            auto node6_prev = static_cast<test::Node<Obj1, 2, 0>&>(node6).Link();
            ...
            auto cnode6_next = cnode6.Link();
            auto cnode6_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode6).Link();
            auto cnode8_next = cnode8.Link();
            auto cnode8_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode8).Link();
            ...
            assert(cnode8_next == &node1);
            assert(cnode8_prev == &node3);
            assert(cnode6_next == nullptr);
            assert(cnode6_prev == nullptr);
        }

        test::Node<Obj1, 2> node9;
        const test::Node<Obj1, 2>& cnode9 = node9;
        node9 = Obj1{9};
        {
        ...
        const test::Node<Obj1, 2>& cnode10 = node10;
        node10 = std::move(obj2);
        {
            auto node10_next = node10.Link();
            auto node10_prev = static_cast<test::Node<Obj1, 2, 0>&>(node10).Link();
            auto cnode10_next = cnode10.Link();
            auto cnode10_prev = static_cast<const test::Node<Obj1, 2, 0>&>(cnode10).Link();
            assert(node10->val1 == 100);
            assert(node10_next == nullptr);
            assert(node10_prev == nullptr);
            assert(cnode10_next == nullptr);
            assert(cnode10_prev == nullptr);
        }
