...
int main()
{
    using namespace test::out::base::deleg;
    {
        ...
        StatusType s2;
        assert(s2.Bad(StatusType::task_and_output_nullptr) == true);
        assert(s2.IsGood() == false);
        assert(s2.IsBad() == true);
        assert(s2.GetBadCode() == StatusType::task_and_output_nullptr);
        ...
        assert(s3.GetBadCode() == StatusType::task_and_output_nullptr);

        StatusType s4{std::move(s2)};
        
        assert(s4.IsGood() == false);
        assert(s4.IsBad() == true);
        assert(s4.GetBadCode() == StatusType::task_and_output_nullptr);
        ...
        assert(s5.GetBadCode() == StatusType::task_and_output_nullptr);
        ...
        assert(s6.GetBadCode() == StatusType::task_and_output_nullptr);
