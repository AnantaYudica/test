    ...
    TAValue& overflow_out)
{
    ...
    for (TSize i = 0; i < NA; ++i)
    {
        ...
        for (TSize j = 0; j < NB; ++j)
        {
            ...
            if (ij < NA)
            {
                auto c = test::math::integer::Addition<TElementValue,
                    TAValue, TExpandValue, TSize, NA,
                    FSetAValue, FGetAValue, FSplitValue, 
                    FElementValue, FCarryValue, 
                    VElementDefault>(sum, mul, 0, ij);
                if (c != 0)
                    test::math::integer::Addition<TElementValue,
                        TAValue, TExpandValue, TSize, NA,
                        FSetAValue, FGetAValue, FSplitValue, 
                        FElementValue, FCarryValue, 
                        VElementDefault>(overflow_out, 0, c, 0);
            }
            else
            {
                test::math::integer::Addition<TElementValue,
                    ...
                    FSetAValue, FGetAValue, FSplitValue, 
                    FElementValue, FCarryValue, 
                    VElementDefault>(overflow_out, mul, 0, ij - NA);
            }
        ...
        for (TSize j = 0; j < 2; ++j)
        {
            ...
            else
            {
                 test::math::integer::Addition<TElementValue,
                    ...
                    FSetValue, FGetValue, FSplitValue, 
                    FElementValue, FCarryValue, 
                    VElementDefault>(overflow_out, mul, 0, ij - N);
            }
