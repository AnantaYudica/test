...
test::CString<TChar> Integer(const test::fp::Base<10, 
...
test::CString<TChar> Integer(const test::fp::Base<2, 
    test::FloatingPoint<float>, true>& fp_base, 
    const std::size_t& size = 0)
{
    const auto num = fp_base.GetNumber();
...
test::CString<TChar> Integer(const test::fp::Base<NBase, 
    test::FloatingPoint<TFloat>, BNormalize>& fp_base, 
    const std::size_t& size = 0)
{
    const auto cstr = Integer<char, BLower>(fp_base, size);
